      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <span className="text-lime-dark dark:text-lime font-black tracking-[0.2em] uppercase text-sm mb-4 block">
              Opportunities
            </span>
            <h2 className="text-4xl md:text-7xl font-black text-foreground uppercase tracking-tighter leading-none">
              DRIVER <span className="text-lime-dark dark:text-lime transition-colors">POSITIONS</span>
            </h2>
          </div>
          ...
          {positions.map((pos, idx) => (
            ...
            >
              <div className={`relative h-full glass-card p-10 border-foreground/10 ${pos.featured ? 'bg-secondary/40 border-lime/30' : ''} group-hover:bg-secondary/60 transition-all duration-500 flex flex-col`}>
                {pos.featured && (
                  <div className="absolute top-0 right-10 -translate-y-1/2 px-4 py-1 bg-lime text-[#0A0F1A] font-black text-xs uppercase tracking-widest">
                    Hot Job
                  </div>
                )}

                <div className="mb-8">
                  <span className="text-[10px] font-black text-text-secondary uppercase tracking-[0.3em] block mb-2 transition-colors group-hover:text-lime-dark dark:group-hover:text-lime">
                    {pos.type}
                  </span>
                ...
                <div className="space-y-4 mb-12 flex-grow">
                  {pos.features.map((feature, fIdx) => (
                    <div key={fIdx} className="flex items-center gap-3">
                      <CheckCircle2 className="w-5 h-5 text-lime-dark dark:text-lime opacity-50 group-hover:opacity-100 transition-opacity" />
                ...
                <button 
                  ...
                  className={`w-full group/btn relative flex items-center justify-between px-6 py-5 border transition-all duration-300 overflow-hidden ${
                    pos.featured 
                      ? 'bg-lime text-[#0A0F1A] border-lime' 
                      : 'border-foreground/20 text-foreground hover:border-lime group-hover/btn:text-[#0A0F1A] font-bold'
                ...
                >
                  ...
                  {!pos.featured && (
                    <div className="absolute inset-0 bg-lime translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500 ease-out" />
                  )}
                </button>
              </div>
            </div>
