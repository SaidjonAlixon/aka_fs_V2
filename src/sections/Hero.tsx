...
const Hero = ({ className = '', onApplyClick }: HeroProps) => {
  ...
  return (
    <section ref={sectionRef} id="hero" className={`relative w-full h-screen bg-background overflow-hidden ${className}`}>

      {/* Background Canvas */}
      <div className="absolute inset-0 w-full h-full will-change-transform">
        <canvas ref={canvasRef} className="absolute inset-0 block w-full h-full" />
        <div className="absolute inset-0 bg-background/40 backdrop-blur-[2px] md:backdrop-blur-0" />
      </div>
      ...
      >
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-foreground uppercase tracking-tighter leading-[0.9] mb-6">
          FREIGHT <br /> THAT NEVER <br /> STOPS MOVING
        </h1>
        <h2 className="text-foreground font-bold text-lg md:text-xl mb-4 tracking-wide">
          POWERING YOUR FREIGHT. DELIVERING RESULTS.
        </h2>
        <p className="text-text-secondary text-lg md:text-xl max-w-lg mb-8 leading-relaxed font-bold">
      ...
      >
        ...
        <h2 className="text-4xl md:text-7xl lg:text-8xl font-black text-foreground mb-4 uppercase tracking-wider leading-tight">
          Intelligent <br /> Routing
        </h2>
        <p className="text-text-secondary text-lg max-w-[400px] font-bold">
      ...
      >
        <div className="bg-background/80 backdrop-blur-lg p-6 md:p-10 border-l-4 border-l-lime flex flex-col sm:flex-row items-start gap-4 md:gap-6 shadow-2xl relative overflow-hidden ring-1 ring-foreground/5">
          ...
          <div>
            ...
            <p className="text-text-secondary mb-8 text-lg leading-relaxed font-bold">
