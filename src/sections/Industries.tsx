...
const Industries = ({ className = '', onApplyClick }: IndustriesProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="absolute inset-x-6 md:inset-x-auto md:left-[18vw] top-[15vh] md:top-[20vh] md:w-[35vw] max-w-[650px] z-20">
        <div ref={headlineRef} className="will-change-transform">
          ...
          <p className="text-gray-300 text-lg md:text-2xl leading-relaxed max-w-lg mb-8 md:mb-12">
          ...
          <div className="flex justify-start">
            <button 
              ...
              className="group relative inline-flex items-center justify-center px-8 md:px-12 py-4 md:py-5 bg-lime text-navy font-black text-sm md:text-lg uppercase tracking-[0.2em] transform transition-all duration-500 overflow-hidden shadow-[0_0_20px_rgba(184,255,44,0.3)]"
      ...
      <div
        ref={listRef}
        className="absolute inset-x-6 md:inset-x-auto md:right-[8vw] bottom-[10vh] md:top-[18vh] md:w-[40vw] max-w-[650px] z-20"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8 lg:gap-10">
          {industries.map((industry, index) => (
            <div
              key={index}
              className={`industry-item group relative p-1 transition-all duration-500 ${
                index % 2 === 1 ? 'md:mt-12' : ''
              }`}
            >
              <div className="relative glass-card p-4 lg:p-10 border-white/5 hover:border-lime/40 transition-all duration-500 group-hover:-translate-y-4 group-hover:bg-navy-light/90 overflow-hidden">
                {/* Background Accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-lime/5 -mr-12 -mt-12 rounded-full blur-3xl group-hover:bg-lime/20 transition-all duration-500" />
                
                <div className="flex flex-col items-start gap-4 lg:gap-6 relative z-10">
                  <div className="p-3 md:p-5 bg-navy border border-white/10 rounded-sm group-hover:border-lime/50 group-hover:bg-lime/10 transition-all duration-500 transform group-hover:rotate-[360deg]">
                    <industry.icon className="w-6 md:w-8 md:h-8 text-lime" />
                  </div>
                  
                  <div>
                    <h3 className="font-space font-black text-lg lg:text-3xl text-text-primary uppercase tracking-tight mb-1 group-hover:text-lime transition-colors">
                      {industry.name}
                    </h3>
                    <p className="text-gray-400 text-sm lg:text-base leading-snug opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 line-clamp-2">
                      {industry.desc}
                    </p>
                  </div>
